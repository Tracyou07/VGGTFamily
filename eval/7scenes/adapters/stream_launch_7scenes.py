...
def get_args_parser():
    ...
    parser.add_argument("--kf", type=int, default=10, help="keyframe interval")
...
def main(args):
    ...
    with torch.no_grad():
        for name_data, dataset in datasets_all.items():
                    ...
                    if model_name == "stream3r" or "VGGT":
                        ...
                        with torch.cuda.amp.autocast(dtype=dtype):
                            with torch.no_grad():
                                ...
                                torch.cuda.synchronize()
                                inference_time_ms = (time.time() - infer_start) * 1000
            ...
            peak_vram_mb = torch.cuda.max_memory_reserved() / (1024 ** 2)
            print(f"Peak VRAM reserved: {peak_vram_mb:.2f} MiB")
