...
OUT_DIR="$EVAL_DIR/results/vggt_kf${KF}_${RUN_TAG}"
LOG_DIR="$EVAL_DIR/logs"
LOG_FILE="$LOG_DIR/vggt_kf${KF}_${RUN_TAG}.log"
PID_FILE="$LOG_DIR/vggt_kf${KF}_${RUN_TAG}.pid"
VRAM_FILE="$LOG_DIR/vram_vggt_kf${KF}_${RUN_TAG}.csv"
...
"$PYTHON_BIN" -u "$EVAL_DIR/adapters/run_official_vggt.py" \
  --ckpt_path "$CKPT" \
  --output_dir "$OUT_DIR" \
  --data_root "$DATA_ROOT" \
  --kf "$KF" \
