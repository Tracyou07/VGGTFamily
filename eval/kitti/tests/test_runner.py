...
def run_fixture(tmp_path, monkeypatch):
    ...
    raw = tmp_path / "raw"
    (raw / "poses").mkdir(parents=True)
    (tmp_path / "archives").mkdir()
    ids = tuple(f"{i:02d}" for i in range(11))
    poses = np.repeat(np.eye(4)[None], 3, axis=0)
    poses[:, :3, 3] = [[0,0,0], [1,0,0], [0,1,0]]
    for seq in ids:
        images = raw / "sequences" / seq / "image_2"
        images.mkdir(parents=True)
        for i in range(3):
            Image.new("RGB", (16,12), (i,20,30)).save(images / f"{i:06d}.png")
        (images.parent / "times.txt").write_text("0\n0.1\n0.2\n")
        p = "100 0 8 0 0 100 6 0 0 0 1 0"
        (images.parent / "calib.txt").write_text(f"P0: {p}\nP2: {p}\n")
        np.savetxt(raw / "poses" / f"{seq}.txt", poses[:, :3].reshape(3,12))
    (tmp_path / "sequences.txt").write_text("\n".join(ids)+"\n")
...
""")
    ...
    values = dict(schema_version=1, raw_root="raw", archive_root="archives",
                  ...
                  models={"vggt": dict(project_root=str(model), checkpoint=str(checkpoint), interpreter=str(child))})
    path = tmp_path / "config.json"
    path.write_text(json.dumps(values))
    config = load_config(path)
    for seq in ids:
        prepare_sequence(config, seq)
...
def make_plan(f, sequences=("00",)):
    from kitti_eval.runner import preflight_run
...
def test_preflight_is_readonly_and_immutable(run_fixture):
    ...
    assert len(p.prepared_sequences[0].frame_ids) == 3
...
def test_all_preflight_blockers_precede_worker(run_fixture, monkeypatch, blocker):
    ...
    device, ids = "cuda:0", ("00",)
    if blocker == "config": f.config_path.write_text("{}")
    elif blocker == "manifest": (f.config.prepared_root/"00/manifest.json").write_text("{}")
...
def test_resume_quarantines_and_reruns(run_fixture, damage):
    from kitti_eval.runner import run_evaluation
    ...
    if damage == "partial": (d/"result.json").unlink()
        ...
        if damage == "stale": r["provenance_id"] = "0"*64
            ...
            r["input_frames"] = m["matched_frames"] = 4
    ...
    assert summary.per_sequence["00"] < 1e-10
    assert json.loads((d/"worker/request.json").read_text())["request_id"] != original
    assert list((p.output_dir/"failures").glob("00-*-*"))
...
def test_exact_resume_and_nonresume_collision(run_fixture, monkeypatch):
    ...
    monkeypatch.setattr(runner,"launch_worker",lambda *a,**k: pytest.fail("worker started"))
    assert runner.run_evaluation(make_plan(run_fixture),resume=True).per_sequence["00"] < 1e-10
...
def test_all_sequences_rechecked_before_launch(run_fixture, monkeypatch):
    from kitti_eval import runner
    p = make_plan(run_fixture,("00","01"))
    (run_fixture.config.raw_root/"poses/01.txt").write_text("corrupted")
    monkeypatch.setattr(runner,"launch_worker",lambda *a,**k: pytest.fail("worker started"))
...
def test_metric_failure_keeps_terminal_artifact(run_fixture, monkeypatch):
    ...
    assert summary.failures["00"]["status"] == "error"
    failure = json.loads((p.output_dir / "00/failure.json").read_text())
...
def test_run_sequence_accepts_matching_verified_input(run_fixture):
    from kitti_eval.runner import run_sequence
    from kitti_eval.data import verify_prepared_sequence
    p = make_plan(run_fixture)
    prepared = verify_prepared_sequence(run_fixture.config, "00")
...
def test_source_drift_after_success_fails_every_remaining_sequence(run_fixture, monkeypatch):
    ...
    plan = make_plan(run_fixture, ("00", "01", "02"))
    ...
    first = read_json(plan.output_dir / "00/result.json")
...
def test_output_inside_consumed_slam_source_rejected_before_writes(run_fixture, relative):
    ...
    with pytest.raises(ValueError, match="OUTPUT_COLLISION"):
        preflight_run(f.config_path, "vggt_slam", ("00",), output, "cuda:0")
...
def test_cli_interpreter_probe_failures_are_structured_json(run_fixture, monkeypatch, capsys, failure):
    ...
    code = main(["run", "--config", str(run_fixture.config_path), "--model", "vggt",
                 "--sequence", "00", "--output", str(run_fixture.output)])
