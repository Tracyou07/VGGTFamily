...
def cli(*args):
    return subprocess.run([sys.executable, "-B", "-c",
        "import sys; from kitti_eval.cli import main; code=main(sys.argv[1:]); "
...
def test_full_cpu_fake_cli_artifacts(run_fixture):
    ...
    required = ["run_manifest.json","all_sequences_metrics.json","average_metrics.json","summary.json"]
    ...
    worker = read_json(f.output/"00/worker/worker_result.json")
...
def test_subset_success_not_formal_completion(run_fixture):
    f = run_fixture
    c = cli("run","--config",f.config_path,"--model","vggt","--sequence","00","--output",f.output)
    assert c.returncode == 0, c.stdout+c.stderr
    s = json.loads(c.stdout)
    assert s["requested_complete"] and not s["formal_complete"] and not s["complete"]
    assert s["avg_ate_rmse_m"] is None
    assert cli("aggregate","--output",f.output).returncode == 1

def test_failure_continues_and_exits_nonzero(run_fixture):
    f = run_fixture
    f.values["models"]["vggt"]["fixture_fail"] = "00"
    f.config_path.write_text(json.dumps(f.values))
    c = cli("run","--config",f.config_path,"--model","vggt","--sequence","00","01","--output",f.output)
    assert c.returncode == 1, c.stdout+c.stderr
    s = json.loads(c.stdout)
    assert not s["requested_complete"] and s["failures"]["00"]["status"] == "error"
    assert "01" in s["per_sequence"] and (f.output/"00/failure.json").is_file()
...
def test_thin_wrappers_resolve_root_and_preserve_physical_gpu(run_fixture):
    ...
    run = subprocess.run(["bash", str(root / "scripts/run_one.sh"), "vggt", "00",
    ...
    assert "--sequence 00" in run.stderr and "CUDA_VISIBLE_DEVICES=7" in run.stderr
    assert not (run_fixture.output / "01").exists()
