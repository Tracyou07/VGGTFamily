...
output/
  ...
  SS/metrics.json
  SS/result.json
  SS/worker/request.json
  SS/worker/prediction.npz
  SS/worker/worker_result.json
  SS/worker/stdout.log
  SS/worker/stderr.log
  SS/failure.json       # detailed terminal failure, when present
...
| `sequence` | Canonical sequence string, 00–10 |
...
The protocol is `kitti-odometry-ate-sim3-v1`; `rmse_m` is finite nonnegative
camera-center ATE RMSE in metres. `frame_ids_sha256` binds the ordered original IDs.
...
Each `sequences` record includes sequence identity, ordered frame IDs, their hash
and prepared-manifest identity. Runner-resolved model controls also retain the
...
Formal completeness requires valid successes for all eleven canonical sequences
00–10 and no invalid or extra sequence artifacts. `Avg.` requires all eleven;
`Avg.*` requires all ten except 01. Both are arithmetic means of sequence RMSEs,
not a pooled-frame RMSE. `average_metrics.json` stores the validated averages;
an incomplete subset cannot become a formal average.
