# KITTI Odometry verification gates
...
model/dataset trees.
KITTI regressions also prove that the configured Long dependency resolves inside
an isolated repository, that doctor names the missing local target, and that the
offline setup passes only pinned packages and a local target to pip without executing
an installation.

Initial packaging CPU suite snapshot (2026-09-15): **269 passed** with the existing
...
```bash
PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=src "$PYTHON" -B -m kitti_eval doctor --config configs/h20.json
PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=src "$PYTHON" -B -m kitti_eval doctor --config configs/h20.json --model vggt_long
```
...
sequence, then run the lightest ready model serially. No current CLI
...
Inspect original frame IDs and timestamps, pose direction, camera coordinates,
...
After the smoke passes, run sequence 00 for each of the six canonical models,
one GPU job at a time, and inspect each backend contract. Only then deliberately
schedule complete sequences 00–10. Validate all result pairs before accepting
`Avg.` or `Avg.*`; never launch a full matrix automatically.
