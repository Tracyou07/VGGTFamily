...
def resolve_config(model_key, config):
    ...
    if c["use_calibration"] is not False:
        raise ValueError("BACKEND_CONFIG: these native profiles do not consume KITTI calibration")
    ...
    if key == "vggt_long":
        ...
        if any(type(c[n]) is not int or c[n] != v for n, v in
            ...
            raise ValueError("BACKEND_CONFIG: KITTI Long requires chunk 75, overlap 30, loop chunk 20")
        if c["retrieval"] != "salad" or c["using_sim3"] is not True:
            raise ValueError("BACKEND_CONFIG: KITTI Long requires SALAD and Sim3")
...
def doctor_backend(model_key: str, config: Mapping[str, object]) -> BackendStatus:
    ...
    if not interpreter.is_file() or not os.access(interpreter, os.X_OK):
        ...
        try:
            completed = subprocess.run([str(interpreter), "-B", "-m", "kitti_eval.backend_worker", "--doctor"],
