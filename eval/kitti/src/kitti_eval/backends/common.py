...
def original_frame_ids(values):
    ids = tuple(values)
    if (not ids or any(not isinstance(v, str) or re.fullmatch(r"[0-9]{6}", v) is None for v in ids)
        or ids != tuple(sorted(set(ids)))):
        raise ValueError("BACKEND_FRAME_IDS: ordered unique original six-digit IDs required")
    return ids
...
class BackendRequest:
    ...
    def __post_init__(self):
        ...
        if (not isinstance(self.request_id, str) or not self.request_id
            ...
            or re.fullmatch(r"(0[0-9]|10)", self.sequence) is None
