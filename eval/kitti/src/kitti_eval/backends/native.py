...
class VGGTOmegaBackend(_Backend):
    def load(self):
        ...
        spec = importlib.util.spec_from_file_location("_kitti_native_omega_run", root / "run.py")
...
class VGGTLongBackend(_Backend):
    def load(self):
        ...
        config["Weights"].update(model="VGGT", VGGT=self.config["checkpoint"],
            SALAD=self.config["salad_checkpoint"], DNIO=self.config["dino_checkpoint"])
        config["Model"].update(chunk_size=75, overlap=30, loop_chunk_size=20, loop_enable=True,
        ...
        config["Model"]["Pointcloud_Save"].update(use_confidence_filtering=True,
            conf_threshold_coef=self.config["conf_threshold_coef"])
        config["Loop"]["SIM3_Optimizer"]["lang_version"] = "python"
        config["Loop"]["SALAD"]["batch_size"] = self.config["salad_batch_size"]
