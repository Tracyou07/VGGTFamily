...
def _snapshot_prepared(prepared: PreparedSequence) -> PreparedSequence:
    return PreparedSequence(
        ...
        timestamps_s=_snapshot_array(prepared.timestamps_s),
...
def _check_manifest(record, config):
    ...
    dataset = KittiConfig(
        ...
        sequences_file=Path(config["sequences_file"]),
        sequence_ids=tuple(config["sequence_ids"]),
        color_root=Path(config.get("color_root", raw_root)),
        aux_root=Path(config.get("aux_root", raw_root)),
    )
...
def build_provenance(plan: RunPlan) -> dict[str, JSONValue]:
    ...
    ids = validate_frame_ids(plan.sequence_ids)
    if any(len(s) != 2 or not s.isascii() or not s.isdigit() or int(s) > 10 for s in ids):
        raise ValueError("PROVENANCE_INVALID: KITTI sequences must be 00 through 10")
...
def validate_provenance(payload, *, check_sources: bool = True) -> None:
    try:
        ...
        ids = validate_frame_ids(data["sequence_ids"])
        if any(len(s) != 2 or not s.isascii() or not s.isdigit() or int(s) > 10 for s in ids):
            raise ValueError("invalid KITTI sequence IDs")
        ...
        for field in ("color_root", "aux_root"):
            if field in config and (not isinstance(config[field], str) or not Path(config[field]).is_absolute()):
                raise ValueError("normalized data roots require absolute paths")
