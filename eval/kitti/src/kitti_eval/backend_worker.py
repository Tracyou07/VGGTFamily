...
def launch_worker(request: BackendRequest, timeout_s: float) -> WorkerResult:
    ...
    try:
        ...
        command = [request.model_config["interpreter"], "-B", "-m", "kitti_eval.backend_worker",
...
def _doctor_probe(payload):
    ...
    active = [True]
    with tempfile.TemporaryDirectory(prefix="kitti-doctor-") as directory:
...
def main(argv=None):
    parser = argparse.ArgumentParser(prog="kitti-backend-worker")
