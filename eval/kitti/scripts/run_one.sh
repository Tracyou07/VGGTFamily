...
if [[ ! "$sequence" =~ ^(0[0-9]|10)$ ]]; then
  printf 'SEQUENCE must be one explicit KITTI ID (00 through 10)\n' >&2
  exit 2
fi
for argument in "$@"; do
...
command=("${PYTHON:-python3}" -B -m kitti_eval run --config "$root/configs/h20.json"
