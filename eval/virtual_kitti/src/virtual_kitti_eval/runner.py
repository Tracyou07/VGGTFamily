...
from .config import load_config, ALLOWED_SEQUENCES
...
ROOT_ARTIFACTS = {"run_manifest.json", "all_sequences_metrics.json",
                  "summary.json"}
SCENE_PATTERN = re.compile(r"Scene(?:01|02|06|18|20)")
...
def _validate_output(output, config, model, config_path):
    ...
    protected = [config.raw_root, config.archive_root, config.prepared_root,
                 config_path, config.sequences_file]
    ...
    if output.exists():
        ...
        for child in output.iterdir():
            ...
            if child.is_file() and child.name in ROOT_ARTIFACTS:
                continue
            if child.is_dir() and child.name == "failures":
                continue
            if child.is_dir() and SCENE_PATTERN.fullmatch(child.name):
                for condition in child.iterdir():
                    sequence = f"{child.name}/{condition.name}"
                    if condition.is_symlink() or not condition.is_dir() or sequence not in ALLOWED_SEQUENCES:
                        raise ValueError(f"OUTPUT_COLLISION: unrecognized or aliased condition {sequence}")
                continue
...
def preflight_run(config_path: Path, model_key: str, sequence_ids: Sequence[str],
    ...
    payload = {name: str(getattr(config, name)) for name in
               ("raw_root", "archive_root", "prepared_root", "sequences_file")}
    payload.update(schema_version=config.schema_version, dataset_version=config.dataset_version,
                   camera=config.camera, sequence_ids=list(config.sequence_ids),
                   execution_environment=_execution_environment())
    command = ("virtual-kitti-eval", "run", "--config", str(config_path), "--model", key,
...
def run_evaluation(plan: RunPlan, resume: bool = False) -> AggregateSummary:
    ...
    try:
        ...
        for scene in sorted(plan.output_dir.iterdir()):
            if scene.is_dir() and SCENE_PATTERN.fullmatch(scene.name):
                for directory in sorted(scene.iterdir()):
                    if f"{scene.name}/{directory.name}" not in plan.sequence_ids:
