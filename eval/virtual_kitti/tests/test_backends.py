...
def test_model_keys_normalize_to_six_native_models(value, expected):
    from virtual_kitti_eval.backends import normalize_model_key
...
def prediction(**changes):
    from virtual_kitti_eval.backends.common import BackendPrediction
    values = dict(frame_ids=("00003", "00010"), poses_c2w=np.repeat(np.eye(4)[None], 2, axis=0),
...
def test_prediction_accepts_rigid_original_ids_and_optional_world_points():
    assert prediction().frame_ids == ("00003", "00010")
...
@pytest.mark.parametrize("change", [
    ...
    {"frame_ids": ("00010", "00003")}, {"frame_ids": ("3", "10")}])
...
def test_requested_frame_order_must_match():
    from virtual_kitti_eval.backends.common import validate_prediction
    with pytest.raises(ValueError, match="FRAME"):
        validate_prediction(prediction(), ("00003", "00009"))


def test_w2c_conversion_is_explicit_and_does_not_absorb_scale():
    from virtual_kitti_eval.backends.common import w2c_to_c2w
...
import virtual_kitti_eval.backends
...
def test_config_loads_exact_six_models_and_immutable_nested_controls(vkitti_131_fixture):
    ...
    assert Path(long["dependency_path"]).resolve() == Path("/home/ubuntu/yjh/feedforwardreconstruct/vggtlong/.runtime/long_deps")
    with pytest.raises(TypeError):
        long["chunk_size"] = 60
    assert config.models["vggt_slam"]["interpreter"].endswith("/monst3r/bin/python")
    assert vkitti_131_fixture.config.models == {}


def test_doctor_rejects_missing_files_without_launching_model(tmp_path):
    from virtual_kitti_eval.backends import doctor_backend
...
def test_checkpoint_validation_catches_truncation_and_lfs_pointer(tmp_path):
    from virtual_kitti_eval.backends.common import inspect_checkpoint
...
def test_long_reuses_preloaded_model_and_retrieval_inside_timing(monkeypatch):
    from virtual_kitti_eval.backends.native import preload_long_models
    from virtual_kitti_eval.resources import measure_inference
    events = []
    class Model:
    ...
    monkeypatch.setattr("virtual_kitti_eval.resources._smi", lambda label: {})
    measure_inference(Backend(), ("00003", "00010"), (Path("a"), Path("b")), Cuda(), lambda: 0.)
...
def test_long_chunk_scale_transforms_translation_but_not_rotation():
    from virtual_kitti_eval.backends.common import extract_long_chunks
    chunks = []
    for _ in range(2):
...
def _exercise_native_forward(key, tmp_path, monkeypatch):
    ...
    import torch
    from virtual_kitti_eval.backends.native import create_backend
    from virtual_kitti_eval.resources import measure_inference
    events = []
    ...
    def preprocess(paths):
        assert paths == [str(tmp_path / "00003.png"), str(tmp_path / "00010.png")]
    ...
    monkeypatch.setattr("virtual_kitti_eval.backends.native._Backend._begin_load", lambda self: None)
    monkeypatch.setattr("virtual_kitti_eval.backends.native.load_state", lambda path: {"weight": 1})
    def checked(model, state):
        events.append("weights")
        return {"loaded_keys": 1}
    monkeypatch.setattr("virtual_kitti_eval.backends.native.checked_load", checked)
    ...
    prediction, _ = measure_inference(backend, ("00003", "00010"),
        (tmp_path / "00003.png", tmp_path / "00010.png"), Cuda(), lambda: 0.)
...
def test_native_namespace_collision_is_rejected_before_path_install(tmp_path, monkeypatch):
    import types
    from virtual_kitti_eval.backends.native import install_source
...
def test_virtual_kitti_controls_are_explicit_and_applied_to_native_config():
    from virtual_kitti_eval.config import load_config
    from virtual_kitti_eval.backends.native import long_native_config
    path = Path(__file__).parents[1]/"configs/h20.json"
    model = load_config(path).models["vggt_long"]
    raw = json.loads(path.read_text())["models"]["vggt_long"]
    assert {"chunk_size","overlap","loop_chunk_size","loop_closure","retrieval","using_sim3"} <= raw.keys()
    base = {"Weights":{},"Model":{"IRLS":{},"Pointcloud_Save":{}},"Loop":{"SIM3_Optimizer":{},"SALAD":{}}}
    resolved = long_native_config(base, model)
    assert (resolved["Model"]["chunk_size"],resolved["Model"]["overlap"],resolved["Model"]["loop_chunk_size"]) == (75,30,20)
    assert resolved["Model"]["loop_enable"] is True


def test_request_reads_only_rgb_frame_identity_from_prepared(vkitti_131_fixture,tmp_path):
    from virtual_kitti_eval.backends.common import BackendRequest
    from virtual_kitti_eval.data import prepare_sequence
    prepared = prepare_sequence(vkitti_131_fixture.config,"Scene01/Clone")
    class RgbOnly:
        frame_ids, image_paths, sequence = prepared.frame_ids,prepared.image_paths,prepared.sequence
        def __getattr__(self,name):
            raise AssertionError("backend requested a forbidden prepared field: "+name)
    request = BackendRequest.from_prepared(RgbOnly(),model_key="vggt",model_config={
        "project_root":str(tmp_path),"checkpoint":str(tmp_path/"weights"),"interpreter":sys.executable},
        output_dir=tmp_path/"worker",provenance_id="f"*64)
    assert set(request.to_dict()) == {"schema_version","model_key","model_config","frame_ids","image_paths",
        "output_dir","request_id","provenance_id","sequence","device"}
    assert request.frame_ids == ("00000","00001","00002","00003")
