...
def test_measurement_excludes_load_and_snapshot_time_and_covers_native_pipeline(monkeypatch):
    ...
    class Backend:
        ...
        def infer(self, frame_ids, image_paths):
            assert frame_ids == ("00007",) and image_paths == (Path("00007.png"),)
    ...
    def smi(label):
        ...
        time[0] += 7
        return {"label": label}
    monkeypatch.setattr("virtual_kitti_eval.resources._smi", smi)
    prediction, usage = measure_inference(Backend(), ("00007",), (Path("00007.png"),), Cuda(), clock)
    ...
    assert prediction.frame_ids == ("00007",)
...
def test_resource_numbers_are_finite_nonnegative(field, value):
    from virtual_kitti_eval.resources import ResourceUsage
