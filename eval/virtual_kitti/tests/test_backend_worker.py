...
""")
    ...
    return BackendRequest(model_key="vggt", model_config={"interpreter": str(child),
        ...
        frame_ids=("00007",), image_paths=(image,), output_dir=tmp_path / "output",
        request_id="req-1", provenance_id="a" * 64, sequence="Scene01/Clone", device="cuda:0")


def test_request_keeps_prepared_original_frame_ids_and_decoded_paths(vkitti_131_fixture, tmp_path):
    from virtual_kitti_eval.backends.common import BackendRequest
    ...
    assert request.sequence == "Scene01/Clone"
...
def test_parent_launches_configured_interpreter_and_validates_output(fake_request):
    from virtual_kitti_eval.backend_worker import launch_worker
    result = launch_worker(fake_request, timeout_s=10)
    assert result.status == "success", result.message
    assert result.prediction.frame_ids == ("00007",)
...
def test_worker_main_writes_atomic_prediction_and_resource_envelope(fake_request, monkeypatch):
    import types
    from virtual_kitti_eval import backend_worker
    from virtual_kitti_eval.backends.common import BackendPrediction
    from virtual_kitti_eval.results import atomic_write_json, read_json
    events = []
    class Backend:
    ...
    monkeypatch.setattr("virtual_kitti_eval.backends.native.create_backend", lambda *a, **k: Backend())
    monkeypatch.setattr("virtual_kitti_eval.resources._smi", lambda label: {"label": label})
    ...
    with np.load(fake_request.output_dir / "prediction.npz", allow_pickle=False) as saved:
        assert saved["frame_ids"].tolist() == ["00007"]
...
def test_corrupt_zip_is_a_structured_output_failure(fake_request):
    from virtual_kitti_eval.backend_worker import launch_worker, _load_success
    import hashlib
    result = launch_worker(fake_request, timeout_s=10)
...
def test_worker_redirects_native_caches_out_of_external_repositories(fake_request, monkeypatch):
    import types
    from virtual_kitti_eval.backend_worker import execute_request
    from virtual_kitti_eval.backends.common import BackendPrediction
    observed = {}
    class Backend:
        def load(self):
