# Vendored from FastVGGT vggt/utils/eval_utils.py (SHA-256
# d7d6a515c071b4b4e3c1d4cf89d728216fd7c3bec2dd119a59706f25bc9728e8).
# Runtime-only deviations: force a headless plotting backend, lazy-load the two
# model-namespace helpers inside inference, accept an explicit GT PLY path, and
# preserve the pinned official evo 1.32 dual-alignment semantics on newer evo.
...
def load_gt_pointcloud(scene_id, gt_ply_dir):
    gt_ply_dir = Path(gt_ply_dir)
    if gt_ply_dir.is_file():
        ply_path = gt_ply_dir
    else:
...
def _evo_metric_with_evo132_alignment(metric_function, traj_ref, traj_est, **kwargs):
    """Run evo while preserving official evo 1.32 dual-alignment order.

    FastVGGT passes both ``align=True`` and ``align_origin=True``. Official evo
    1.32 first applies Sim(3) alignment and then aligns the first pose. Newer evo
    rejects that combination before mutation. Recreate the 1.32 order only on
    that exact compatibility error.
    """

    try:
        return metric_function(traj_ref, traj_est, **kwargs)
    except ValueError as error:
        if (
            str(error) != "align and align_origin can't be used simultaneously"
            or not kwargs.get("align")
            or not kwargs.get("align_origin")
        ):
            raise
    traj_est.align(
        traj_ref,
        correct_scale=bool(kwargs.get("correct_scale")),
        correct_only_scale=False,
        n=int(kwargs.get("n_to_align", -1)),
    )
    traj_est.align_origin(traj_ref)
    compatible_kwargs = dict(kwargs)
    compatible_kwargs["align"] = False
    compatible_kwargs["correct_scale"] = False
    compatible_kwargs["align_origin"] = False
    return metric_function(traj_ref, traj_est, **compatible_kwargs)
...
def eval_trajectory(poses_est, poses_gt, frame_ids, align=False):
    ...
    ate_result = _evo_metric_with_evo132_alignment(
        main_ape.ape,
        deepcopy(traj_ref),
        deepcopy(traj_est),
        est_name="traj",
    ...
    are_result = _evo_metric_with_evo132_alignment(
        main_ape.ape,
        deepcopy(traj_ref),
        deepcopy(traj_est),
        est_name="traj",
    ...
    rpe_rots_result = _evo_metric_with_evo132_alignment(
        main_rpe.rpe,
        deepcopy(traj_ref),
        deepcopy(traj_est),
        est_name="traj",
    ...
    rpe_transs_result = _evo_metric_with_evo132_alignment(
        main_rpe.rpe,
        deepcopy(traj_ref),
        deepcopy(traj_est),
        est_name="traj",
...
def infer_vggt_and_reconstruct(
    ...
    # Lazy imports keep this evaluator usable beside other top-level ``vggt``
    # packages. Only model inference needs FastVGGT's model-specific helpers.
    ...
    # Quaternion arithmetic in BF16 yields nonorthogonal rotations (observed
    # max |R R^T-I|=0.008 in the real smoke). Decode in FP32 before BOTH depth
    # unprojection and pose export, preserving their common camera geometry.
    # Model parameters and the model forward above remain BF16.
    extrinsic, intrinsic = pose_encoding_to_extri_intri(
        predictions["pose_enc"].float(), (vgg_input.shape[2], vgg_input.shape[3])
    )
