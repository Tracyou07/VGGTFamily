"""One-mode, arbitrary-length H20 launcher for independent v6 run IDs."""
import argparse
import fcntl
import json
import os
...
from .runtime import (ROOT, data_identity, fresh_directory, prepare_inputs,
                      preflight, sha256, source_identity, write_json)
from vggt.v6.attention import MODES
...
def main():
    ...
    output_root = Path("/data/yjh/output/vggt/ours_v6").resolve()
    if not args.output.is_absolute() or not args.output.resolve().is_relative_to(output_root):
        parser.error("--output must be a new path inside /data/yjh/output/vggt/ours_v6")
    ...
    if not identity["commit"] or identity["status"]:
        raise RuntimeError("ours_v6 requires a committed clean tree")
    ...
    try:
        ...
        try:
            ...
            command = [sys.executable, "-B", "-u", "-m", "experiments.ours_v6.worker",
