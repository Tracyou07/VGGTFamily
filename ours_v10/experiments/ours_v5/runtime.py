...
def source_identity():
    ...
    extra=[str(p.relative_to(ROOT)) for folder in ('vggt/v5','experiments/ours_v5','vendor/vggtlong') for p in (ROOT/folder).rglob('*') if p.is_file() and '__pycache__' not in str(p)]
...
def data_identity(scene,frame_list):
    ...
    if len(ids)!=len(set(ids)) or len(ids)<100:raise ValueError('expected fixed list with >=100 unique frames')
