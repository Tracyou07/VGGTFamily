"""One BF16 CUDA v6 reconstruction process for one v7 communication mode."""
import argparse
import json
import os
import sys
...
def execute(args):
    ...
    if not source.get('commit') or source.get('status'):
        raise RuntimeError('ours_v7 needs a committed, clean working tree')
    ...
    manifest["exchange_sdpa_backend"]=dict(requested=args.exchange_sdpa_backend,
        scope="v7 exchange_attention only",serial_inference_only=True)
    ...
    # Supported only in this serial, dedicated inference process. The cache
    # temporarily replaces module functions; concurrent inference is unsupported.
    key_context=cached_sdpa_keys() if getattr(args,"cache_sdpa_keys",False) else nullcontext({})
    ...
    with torch.inference_mode(),torch.autocast('cuda',dtype=torch.bfloat16),key_context as key_stats:
        result=WindowReconstructor(model)(images,ids,mode=args.mode,window_size=args.window_size,overlap=args.overlap,
                                          query_chunk_size=args.query_chunk_size or config['query_chunk_size'],
                                          local_query_chunk_size=args.local_query_chunk_size,
                                          cross_query_chunk_size=args.cross_query_chunk_size,
                                          reuse_image_encoding=args.reuse_image_encoding,
                                          patch_exchange_ratio=args.patch_exchange_ratio,
                                          dense_head_frame_chunk=getattr(args,'dense_head_frame_chunk',None),
                                          exchange_sdpa_backend=args.exchange_sdpa_backend)
    torch.cuda.synchronize();forward=time.perf_counter()-start
    manifest["patch_selection"] = result["patch_selection"]
    manifest["communication_mode"] = args.mode
    manifest["patch_exchange_ratio"] = args.patch_exchange_ratio
    manifest["patch_selection_algorithm"] = result["patch_selection"]["algorithm"]
    ...
    if whole_task:
        allocated=torch.cuda.max_memory_allocated();reserved=torch.cuda.max_memory_reserved()
    manifest["optimization"]=dict(backend_profile=profile,exchange_sdpa_backend=args.exchange_sdpa_backend,
        legacy_query_chunk_size=args.query_chunk_size or config['query_chunk_size'],
        local_query_chunk_size=args.local_query_chunk_size or config['query_chunk_size'],
        cross_query_chunk_size=args.cross_query_chunk_size or config['query_chunk_size'],
        reuse_image_encoding=args.reuse_image_encoding,
        dense_head_frame_chunk=getattr(args,"dense_head_frame_chunk",None),
        cache_sdpa_keys=getattr(args,"cache_sdpa_keys",False),key_cache_scope="serial dedicated process only",
        key_cache_stats=key_stats,npz_compression_level=getattr(args,"npz_compression_level",6),
...
def parse_group_chunk(value):
    if value == 'all':
        return value
    try:
        size = int(value)
    except ValueError as exc:
        raise argparse.ArgumentTypeError('query chunk must be positive or all') from exc
    if size < 1:
        raise argparse.ArgumentTypeError('query chunk must be positive or all')
    return size
...
def parse_args(argv=None):
    ...
    parser.add_argument('--mode',choices=['independent','camera_exchange','camera_patch_exchange'],required=True)
    parser.add_argument('--patch-exchange-ratio',type=float,default=0.10)
    parser.add_argument('--dense-head-frame-chunk',type=int,default=None)
    parser.add_argument('--cache-sdpa-keys',action='store_true',help='Serial dedicated process only')
    ...
    parser.add_argument('--exchange-sdpa-backend',choices=('auto','flash'),default='auto')
    parser.add_argument('--query-chunk-size',type=int,default=None)
    parser.add_argument('--local-query-chunk-size',type=parse_group_chunk,default=None)
    parser.add_argument('--cross-query-chunk-size',type=parse_group_chunk,default=None)
    ...
    if args.query_chunk_size is not None:
        ...
        if args.local_query_chunk_size is not None or args.cross_query_chunk_size is not None:
            parser.error('--query-chunk-size conflicts with group-specific query chunks')
    ...
    if not 0 <= args.patch_exchange_ratio <= 1:
        parser.error('--patch-exchange-ratio must be in [0,1]')
    return args
