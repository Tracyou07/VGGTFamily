...
class ModelTest(unittest.TestCase):
    ...
    def test_original_heads_with_required_prediction_cache(self):
        ...
        out=wrapper(self.images,list('abcde'),mode='independent',window_size=3,overlap=1)
    ...
    def test_all_single_window_modes_match_original_outputs(self):
        with torch.inference_mode():
            original = self.m(self.images[None])
        reference = None
        for mode in ("independent", "camera_exchange", "camera_patch_exchange"):
            pred = WindowReconstructor(self.m)(
                self.images, list("abcde"), mode=mode, window_size=5, overlap=1
            )["predictions"][0]
            for output_key, original_key in (
                ...
                torch.testing.assert_close(pred[output_key], original[original_key][0],
                                           atol=2e-5, rtol=2e-5)
            if reference is None:
                reference = pred
            else:
                for key in ("c2w", "intrinsics", "depth", "depth_conf",
                            "world_points", "world_points_conf"):
                    torch.testing.assert_close(pred[key], reference[key], atol=0, rtol=0)
    ...
    def test_camera_exchange_runs_and_input_contract(self):
        ...
        self.assertEqual(result['patch_selection']['selected_count'], 2)
