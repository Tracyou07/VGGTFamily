...
set -euo pipefail
if [[ $# -lt 4 ]]; then
  echo "Usage: bash scripts/run_v7.sh GPU_ID MODE FRAMES OUTPUT [--frame-list PATH] [--scene-root PATH]" >&2
  exit 2
fi
...
exec "$PYTHON" -B -u -m experiments.ours_v7 run --gpu "$GPU_ID" --mode "$MODE" --frames "$FRAMES" --output "$OUTPUT" "$@"
