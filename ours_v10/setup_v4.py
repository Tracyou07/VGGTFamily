...
r=Path('/home/ubuntu/yjh/feedforwardreconstruct/ours_v4')
source=(r/'experiments/ours_v3/runner.py').read_text()
parts=[ast.get_source_segment(source,n) for n in ast.parse(source).body if isinstance(n,ast.FunctionDef) and n.name in ('write_cloud','evaluate')]
(r/'experiments/ours_v4/artifacts.py').write_text('from pathlib import Path\nimport numpy as np\nfrom experiments.ours_v3.geometry import fit_sim3,unproject_pixels\nfrom experiments.ours_v3.stitch import json_write\n\n'+'\n\n'.join(parts)+'\n')
for name in ('test_core.py','test_stitch.py'):
    content=subprocess.check_output(['git','show','d54c6e2:tests/ours_v3/'+name],cwd=r.parent/'ours_v3')
    (r/'tests/ours_v4'/name).write_bytes(content)
(r/'configs/v4_validation.json').write_text(json.dumps(dict(scene_root='/data/yjh/share/datasets/ScanNet/prepared_scannet50_v1/scene0150_00',window_size=30,overlap=10,window_batch_size=2,gate_frames=55,diagnostic_frames=100,tolerances=dict(fp32=dict(atol=0.0005,rtol=0.0005,center_m=0.0001,rotation_deg=0.02),bf16=dict(atol=0.02,rtol=0.02,center_m=0.01,rotation_deg=0.5)),tolerance_reason='Predeclared elementwise FP32 5e-4 and BF16 2e-2 numerical budgets, plus stricter physical pose checks. Reference repeat measures actual noise; budgets are not adjusted after results. Relative denominator floor is atol for reporting only. No Sim3 before equivalence.'),indent=2))
(r/'README_OURS_V4.md').write_text('''# ours_v4: independent packed VGGT* windows
...
CUDA_VISIBLE_DEVICES=1 CUBLAS_WORKSPACE_CONFIG=:4096:8 OMP_NUM_THREADS=2 \
 "$PYTHON" -B -u -m experiments.ours_v4.validate \
 --output /data/yjh/output/vggt/ours_v4/${RUN_ID}
```
