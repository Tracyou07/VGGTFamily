"""Frozen VGGT* window wrapper; each window enters heads separately."""
import time
import torch
...
class WindowReconstructor(nn.Module):
    ...
    def forward(self, images, frame_ids, mode="camera_register_exchange",
                window_size=60, overlap=30, query_chunk_size=64):
        if self.training or self.model.training:
            raise ValueError("v6 requires eval mode")
        ...
        features, patch_start, memory = aggregate_windows(
            self.model.aggregator, images, windows, mode, query_chunk_size
        )
        synchronize(device)
