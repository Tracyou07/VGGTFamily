"""Unchanged v8 frame schedule with v10's opt-in global camera exchange."""
import torch
...
def aggregate_windows(aggregator, images, frame_ids, windows, mode,
    ...
    for layer in range(aggregator.depth):
        ...
        if layer in aggregator.cached_layer_indices:
            for i, (lo, hi) in enumerate(windows):
                ...
                features[i][layer] = merged.cpu() if offload_head_features else merged
                del merged
    ...
    summary["camera_bank"] = dict(
        duplicate_window_frame_instances=correspondence["duplicate_window_frame_instances"],
        max_remote_tokens=max((bank["remote_tokens"] for bank in correspondence["camera_bank"]),
                              default=0) if mode in ("camera_only", "camera_global_overlap") else 0,
        per_window=[dict(remote_tokens=bank["remote_tokens"],
                         remote_duplicate_frame_instances=bank["remote_duplicate_frame_instances"],
                         remote_also_local_frame_instances=bank["remote_also_local_frame_instances"])
                    for bank in correspondence["camera_bank"]]
            if mode in ("camera_only", "camera_global_overlap") else [])
