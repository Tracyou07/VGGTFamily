"""Frozen VGGT* v7 wrapper; each independent window enters heads separately."""
import time
import torch
...
class WindowReconstructor(nn.Module):
    ...
    def forward(self, images, frame_ids, mode="camera_patch_exchange",
                window_size=60, overlap=30, query_chunk_size=64, patch_exchange_ratio=0.10,
                dense_head_frame_chunk=None, exchange_sdpa_backend="auto",
        ...
        if self.training or self.model.training:
            raise ValueError("v7 requires eval mode")
        ...
        features, patch_start, memory, selection = aggregate_windows(
            self.model.aggregator, images, windows, mode, patch_exchange_ratio, query_chunk_size,
        ...
        return dict(predictions=predictions, windows=windows, memory=memory, patch_selection=selection,
                    ...
                    state_device=str(device), cpu_offload=False)
